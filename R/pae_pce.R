...
pae_pce <- function(preabsMat, shapeFile, resol, N = NULL,
  ...
  # Setup hatching angles for different iterations
  if (use_hatching && is.null(hatching_angles)) {
    hatching_angles <- c(45, 135, 90, 0, 45, 135, 90, 0)  # Cycle through 4 distinct patterns
  }

  min_sobreposicao <- 1.0

  if (!is.null(random_seed) && is.finite(random_seed)) {
  ...
  tempMatrix <- phangorn::as.phyDat(preabsMat, type = 'USER', levels = c(0, 1))
  ...
  # Validate geometry: disable S2, make valid, filter remaining invalid geometries
  sf::sf_use_s2(FALSE)
  if (inherits(shapeFile, "SpatVector")) {
    shapeFile_sf <- st_as_sf(shapeFile)
  } else if (inherits(shapeFile, "sf")) {
    shapeFile_sf <- shapeFile
  } else {
    # Spatial* objects
    shapeFile_sf <- st_as_sf(shapeFile)
  }
  shapeFile_sf <- st_make_valid(shapeFile_sf)
  shapeFile_sf <- shapeFile_sf[st_is_valid(shapeFile_sf), ]
  ...
  # Convert validated sf to Spatial for raster operations
  ...
  # NEW: Track grid frequency across all iterations
  grid_frequency_tracker <- list()
  ...
  while(deve_continuar && length(which((ciVec == 1) == TRUE)) > 1 && dim(preabsMat)[2] > 1){
    ...
    if(length(unique(frameTempT$spp)) > 1){
      ...
      if(use_grid_mapping) {
        ...
        if(length(endemic_grid_ids) > 0) {
          ...
          # Plot ALL grid polygons with color intensity increasing per iteration
          # Each iteration adds a darker shade of gray to show frequency
          # Iteration 1 = light gray, Iteration 2 = medium gray, Iteration 3 = dark gray, etc.
          
          # Create grayscale intensity based on iteration (0 = white, 1 = black)
          # Scale from 0.2 (light) to 0.9 (very dark) based on iteration
          max_iterations <- 10  # Assume max 10 iterations for scaling
          intensity <- 0.2 + ((contagem - 1) / max_iterations) * 0.7
          intensity <- min(intensity, 0.9)  # Cap at 0.9 to avoid pure black
          
          # Create gray color with this intensity
          gray_color <- rgb(intensity, intensity, intensity)
          
          if(sobrepo == TRUE){
            plot(endemic_grids_sp, add = TRUE,
                 col = gray_color,  # Fill with gray intensity
                 border = 'black', lwd = 1.5)  # Black border for clarity
          } else {
            plot(endemic_grids_sp, add = TRUE,
                 col = gray_color,  # Fill with gray intensity
                 border = 'black', lwd = 1.5)  # Black border for clarity
        ...
        for(j in 1:dim(speciesNumberT)[1]){
          if(speciesNumberT$Freq[j] > 1){
            ...
            # NEW: Track this grid's frequency
            if(!(grid_value %in% names(grid_frequency_tracker))) {
              grid_frequency_tracker[[grid_value]] <- 0
            }
            grid_frequency_tracker[[grid_value]] <- grid_frequency_tracker[[grid_value]] + 1
          }
        }
        ...
        if(gridView == TRUE){
          ...
          if(labelGrid == TRUE){
            ...
            for(i in 1:length(grid_labels)) {
              ...
              if(grid_id %in% names(grid_frequency_tracker)) {
                freq <- grid_frequency_tracker[[grid_id]]
                # Add asterisks: frequency 1 = 1 asterisk, frequency 2 = 2 asterisks, etc.
                asterisks <- paste(rep('*', freq), collapse = '')
                grid_labels[i] <- paste0(grid_id, asterisks)
              }
            }
    ...
    # Add iteration number to nonHomoplastic species list
    if (!is.null(frameTempT) && nrow(frameTempT) > 0) {
      listaR[[contagem]]$iteration <- contagem
    }
  ...
  if(length(grid_frequency_tracker) > 0) {
    ...
    for(freq in frequencies) {
      asterisks <- paste(rep('*', freq), collapse = '')
      label <- paste0('Appeared ', freq, ' time', ifelse(freq > 1, 's', ''), ' (', asterisks, ')')
      legend_labels <- c(legend_labels, label)
    }
    
    legend(x = 'bottomright', 
           legend = legend_labels,
           title = 'How Many Times It Appeared',
           cex = 0.85,
           bg = 'white',
           box.lwd = 2)
  }
